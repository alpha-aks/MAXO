...
export default function NewsSection() {
  return (
    <section style={{ position: 'sticky', top: 0, zIndex: 50, backgroundColor: '#f8f8f8', color: '#000000', paddingTop: '128px', paddingBottom: '128px', paddingLeft: '24px', paddingRight: '24px' }}>
        ...
        {articles.map((item, i) => (
          ...
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', gap: '32px', alignItems: 'center' }}>
              ...
              {/* Middle: Title */}
              <motion.h3 
                style={{ fontSize: 'clamp(1.5rem, 2vw, 2rem)', fontWeight: 300, margin: 0 }}
                whileHover={{ x: 16, transition: { duration: 0.3 } }}
              >
                {item.title}
              </motion.h3>

              {/* Right: Arrow */}
              <div style={{ fontSize: '20px', display: 'flex', justifyContent: 'flex-end' }}>→</div>
            </div>
          </motion.div>
        ))}
        {/* Closing Line */}
